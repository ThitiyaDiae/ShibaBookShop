...
new GAjax({ method: "GET" }).send(
  "https://oas.kotchasan.com/api.php/categories",
  null,
  function(xhr) {
    // debug ผลตอบกลับจาก Server
    console.log(xhr.responseText); // แปลงข้อมูลตอบกลับเป็น JSON Object
    var ds = xhr.responseText.toJSON(), // menu element #categorymenu
      menu = $G("categorymenu");
    if (ds) {
...
function getProduct(id) {
  ...
  new GAjax({ method: "GET" }).send(
    "https://oas.kotchasan.com/api.php/product/" + id,
    null,
    function(xhr) {
      // debug ผลตอบกลับจาก Server
...
function getProducts(id) {
  ...
  // ใช้ Ajax โหลดข้อมูลตามที่เลือก
  new GAjax({ method: "GET" }).send(
    "https://oas.kotchasan.com/api.php/products/" + id,
    "limit=20",
    function(xhr) {
      // debug ผลตอบกลับจาก Server
      console.log(xhr.responseText);
